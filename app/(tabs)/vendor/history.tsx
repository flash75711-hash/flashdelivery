...
export default function VendorHistoryScreen() {
  ...
  const loadOrders = async () => {
    ...
    try {
      ...
      const { data: historyResponse, error: historyError } = await supabase.functions.invoke('get-vendor-history', {
        body: { vendorId: user.id },
      });

      if (historyError) {
        console.error('Error calling get-vendor-history function:', historyError);
      ...
      if (historyResponse?.success) {
        console.log('Vendor history loaded from Edge Function:', {
              ...
              {item.customer && (
                <View style={styles.infoRow}>
                  ...
                  <Text style={styles.infoText}>
                    العميل: {item.customer.full_name || item.customer.phone || 'غير محدد'}
                  </Text>
                </View>
              )}
